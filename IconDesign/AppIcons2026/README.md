...
- `00-chris-muse.png` — primary icon: Chris’s Muse, designed by Chris, with a white dimensional note on red in Light and a pink-red note on charcoal in Dark.
- `18-soft-note.png` — the previous primary icon, retained as an alternate: a cream dimensional eighth note with a soft cast shadow, on a mint-to-aqua gradient.
...
Chris’s Muse preserves the artwork from `00-chris-muse-light-original.jpg` and `00-chris-muse-dark-original.jpg`. It was alternate 13 until it became the primary icon; number 13 is retired with it. The supplied rounded outer rim is removed so platform masking does not create a second edge; the note's dimensional highlights are retained. Its Tinted variant uses a silver-white note on charcoal. The selected original JPEGs are retained alongside the prepared PNGs in `raw/`.
...
tvOS uses the primary Chris’s Muse design in independently composed landscape/parallax assets: the note and its faint cast shadow form the transparent `Front` layer, the red field is the `Back` layer. Compositing `Front` over `Back` reproduces the square plate (residual rms ≈ 3, all on anti-aliased edges), so the shadow is encoded as black-with-alpha rather than baked into the ground. The two are separated on the green channel alone — the red field never rises above G 40 while the milky note stays above G 140 — and the ground is a degree-4 polynomial fit sampled clear of the note and its shadow (rms 0.84), stretched to each landscape canvas. `BrandMark` is the square plate at 256×256, and `DefaultCover` (the tvOS placeholder for songs without artwork) is the same plate at 512×512. The square-icon generator does not produce any of these; the note's own height is 51% of the canvas on the app icon layers and 40.5% on the Top Shelf images.
...
The source inputs live in `raw/`. `18-soft-note*.png` and `15-folded-note*.png` preserve their exact artwork. A `NN-` prefix is the icon's alternate number; `00-` marks whichever design is currently primary. Retired numbers are never reused. In-app preview imagesets are written at 512×512.
