...
extension ImmersiveEffectDrawer {
    ...
    ///
    /// 取的是 `FullscreenEffectCollection.effects`，和设置页、效果面板同一份：
    /// 随界面皮肤提供、而那套皮肤还不能用的效果不进转轮。权益在运行期会变，
    /// 所以每次取都重算，不缓存成常量。
    static var fullscreenCases: [FullscreenPlayerEffect] {
        FullscreenEffectCollection
            .allCases
            .flatMap(\.effects)
            .filter { !$0.isNative }
    }
