...
/// The gear at the home page's top right: settings left the tab bar so that
/// radio and spoken word could have tabs of their own.
...
/// Shown once to people who used the old layout: where radio, spoken word
/// and settings went. New users meet the new layout first and never see it.
struct HomeListeningSpacesIntroCard: View {
    @AppStorage(ListeningSpacesIntroductionPolicy.seenKey) private var hasSeen = false

    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            HStack(spacing: -4) {
                ForEach(ListeningSpace.allCases, id: \.self) { space in
                    Circle()
                        .fill(space.tint)
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("home_spaces_intro_title")
                    .font(.subheadline.weight(.semibold))
                Text("home_spaces_intro_body")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
