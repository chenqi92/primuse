...
struct ContentView: View {
    ...
    private var visibleLibrarySections: [LibrarySection] {
        ...
        // 极简导航仍把它们放在顶栏分类里。
        .filter { navigationMode == .minimal || ($0 != .radio && $0 != .spokenWord) }
    }
        ...
        TabView(selection: searchAwareTabSelection) {
            ...
            Tab(
                navigationMode == .minimal
                    ? String(localized: "library_title")
            ...
            if navigationMode != .minimal, visibleListeningSpaces.contains(.radio) {
                Tab(
            ...
            if navigationMode != .minimal, visibleListeningSpaces.contains(.spokenWord) {
                Tab(
            ...
            // 设置不再占标签栏的一格,从首页右上角的齿轮打开;极简导航的
            // 顶栏仍把设置当作一页,那里保留这一格(系统标签栏在极简里是隐藏的)。
    ...
    /// 标签栏布局下设置是一张弹层;iPad 侧栏与极简导航里它仍是一页。
    private func openSettings() {
        showNowPlaying = false
        if rootLayout == .standardTabs {
    ...
    private func openListeningSpace(_ space: ListeningSpace) {
        ...
        guard navigationMode != .minimal, availableTabs.contains(tab) else {
            if navigationMode == .minimal {
    ...
    private func openLibraryDeepLink(_ link: LibraryDeepLink) {
        ...
        if navigationMode != .minimal, case .section(let section) = link {
